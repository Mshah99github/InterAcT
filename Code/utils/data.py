...
def duplicate_and_sequence_chunking(dir,dup_n_seq_verbose,fp_seq,dup_flag):
    def duplicates_n_sequence_chunks(path,dup_n_seq_verbose,fp_seq,dup_flag):
        data = np.load(path)
                    ...
                    for video_npy_file in os.listdir(action_path):
                        if video_npy_file.endswith('.npy'):
                            ...
                            if action_folder == 'handshaking':
                                frames_data, seq_data, total_frames_bf_dup,total_frames_af_dup,dup,num_seq = duplicates_n_sequence_chunks(npy_file_path,dup_n_seq_verbose,fp_seq,dup_flag)
...
def preprocess_data(directory, selected_data_folder, dup_n_seq_verbose_selection,fp_seq,dup_flag):    
    ...
    if selected_data_folder == 'all_classes_original':
        ...
        if dup_n_seq_verbose_selection == 'True' or dup_n_seq_verbose_selection == 'true' or dup_n_seq_verbose_selection =='T' or dup_n_seq_verbose_selection =='t'or dup_n_seq_verbose_selection =='TRUE':
            if dup_flag == 'True' or dup_flag == 'true' or dup_flag =='T' or dup_flag =='t'or dup_flag =='TRUE':
               Xdata,Ydata = duplicate_and_sequence_chunking(folderpath, True, fp_seq,True)
            else:
                Xdata,Ydata = duplicate_and_sequence_chunking(folderpath, True, fp_seq,False)
        else:
            if dup_flag == 'True' or dup_flag == 'true' or dup_flag =='T' or dup_flag =='t'or dup_flag =='TRUE':
                Xdata,Ydata = duplicate_and_sequence_chunking(folderpath, False, fp_seq,True)
            else:
                Xdata,Ydata = duplicate_and_sequence_chunking(folderpath, False, fp_seq,False)
