...
class PatchClassEmbedding(tf.keras.layers.Layer):
    def __init__(self, d_model, n_patches, pos_emb=None, kernel_initializer='he_normal', **kwargs):
        ...
        self.d_model = d_model
        self.n_tot_patches = n_patches #n_patches + 1
    ...
    def call(self, inputs):
        #print('class embed shape before repeat:',self.class_embed.shape)
        #print('inputs shape before repeat:',inputs.shape)
        x =  tf.repeat(self.class_embed, tf.shape(inputs)[0], axis=0)
        #print('x shape after repeat in class function of Patchclass emb: ',x.shape)
        x = tf.concat((x, inputs), axis=1)
        x = x[:,-inputs.shape[1]:,:]
        #print('x shape after concat in class function of Patchclass emb: ',x.shape)
        ...
        # print('patchclass embedding class_emb shape: ',self.class_embed.shape)
        # print('patchclass embedding x shape: ',x.shape)
        # print('patchclass embedding pe shape: ',pe.shape)
